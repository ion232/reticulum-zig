...
const Fifo = @import("internal/Fifo.zig").Fifo;
const System = @import("System.zig");
const OutgoingPacket = struct {
    packet: Packet,
    source_interface_id: ?Interface.Id,
    target_interface_id: ?Interface.Id,
};

pub const Error = Interfaces.Error || Packet.ValidationError || Identity.Error || EndpointBuilder.Error || Name.Error || Allocator.Error;
...
pub fn init(allocator: Allocator, system: *System, identity: ?Identity, options: Options) Error!Self {
    var endpoint_builder = EndpointBuilder.init(allocator);
    var main_endpoint = try endpoint_builder
        .setIdentity(identity orelse try Identity.random(&system.rng))
        .setDirection(.in)
        .setVariant(.single)
        .setName(try Name.init(options.name, &.{}, allocator))
        .build();
    defer main_endpoint.deinit();

    const endpoints = try Endpoints.init(allocator, &main_endpoint);
    const interfaces = Interfaces.init(allocator, system.*);
    const ratchets = Ratchets.init(allocator, &system.rng);
    const routes = Routes.init(allocator);
    const announces = Announces.init(allocator);
    const packet_filter_capacity = 32768;
    const packet_filter = try PacketFilter.init(allocator, packet_filter_capacity);
...
pub fn addInterface(self: *Self, config: Interface.Config) Error!Interface.Api {
    ...
    return try self.interfaces.add(config, now);
}
...
pub fn process(self: *Self) !void {
    ...
    while (self.outgoing.readItem()) |entry| {
        try self.packetOut(entry.packet, now);
    }
}
...
fn eventsOut(self: *Self, interface: *Interface, pending: *Interfaces.Pending, now: u64) !void {
    while (pending.readItem()) |entry| {
        ...
        switch (event) {
            .packet => |*packet| not_sent = try self.packetOut(interface, packet, now),
            .task => |task| {
                try interface.outgoing.push(.{ .task = task });
                not_sent = false;
            },
        }

        if (not_sent) event.deinit();
    }
}

fn announceTask(self: *Self, interface: *Interface, announce: *Event.In.Task.Announce, now: u64) !void {
...
fn packetIn(self: *Self, interface: *Interface, packet: *Packet, now: u64) !void {
    ...
    if (self.options.transport_enabled and packet.header.purpose != .announce) {
        if (packet.endpoints == .transport) {
            try self.generalTransport(interface.id, packet, now);
        }
        if (packet.header.purpose != .link_request and packet.context != .link_request_proof) {
            try self.linkTransport(interface.id, packet, now);
        }
    }
...
fn plainTask(self: *Self, interface: *Interface, plain: *Event.In.Task.Plain) !void {
    const packet = try interface.packet_factory.makePlain(plain.name, plain.payload);
    try self.interfaces.broadcast(packet);
}

fn packetOut(self: *Self, packet: *Packet, now: u64) !bool {
                ...
                switch (interface.mode) {
                    ...
                    .boundary => if (!self.endpoints.has(&endpoint)) {
                        ...
                        if (egress_control.announce_queue.count() == 0 and now >= egress_control.announce_release_time and interface.bit_rate != null) {
                            ...
                            const transmission_time = (8 * packet.size()) / bit_rate.bitsPerSecond();
                            ...
                            if (egress_control.announce_queue.count() >= Interfaces.EgressControl.max_queued_announces) {
                                ...
                                while (announce_entries.next()) |entry| {
                                    if (std.mem.eql(u8, &entry.announce.endpoints.endpoint(), &packet.endpoints.endpoint())) {
                                ...
                                if (matching_entry) |entry| {
                                    if (packet.payload.announce.timestamp > entry.announce.payload.announce.timestamp) {
                                        try egress_control.announce_queue.update(
                                            entry,
                                            .{
...
fn generalTransport(self: *Self, interface_id: Interface.Id, packet: *Packet, now: u64) !void {
    ...
    // Check the next hop is this node.
    if (!std.mem.eql(u8, &next_hop, our_hash)) return;
    ...
    self.routes.setLastSeen(endpoint, now);
    try self.interfaces.transmit(packet, interface_id, route.origin_interface_id);
}

fn linkTransport(self: *Self, interface_id: Interface.Id, packet: *Packet, now: u64) !void {
    _ = self;
    _ = interface_id;
    _ = packet;
    _ = now;
...
fn announcePacketIn(self: *Self, interface: *Interface, announce: *Packet, now: u64) !void {
    ...
    if (!self.routes.has(endpoint) and try self.interfaces.shouldIngressLimit(interface.id, now)) {
        try self.interfaces.holdAnnounce(interface.id, try announce.clone());
        return;
    }
...
fn shouldDrop(self: *Self, packet: *const Packet) bool {
    ...
    return switch (packet.context) {
        .keep_alive, .resource_request, .resource_proof, .resource, .cache_request, .link_channel => false,
        else => switch (endpoint) {
            .plain, .group => (purpose == .announce or hops > 1),
            else => !(self.packet_filter.has(packet) and purpose == .announce and endpoint == .single),
        },
    };
}
