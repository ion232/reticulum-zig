...
pub const Aes = struct {
    ...
    pub fn encrypt(dst: []u8, src: []u8, key: [Aes128.block.block_length]u8, iv: [AesEncryptCtx.block_length]u8) void {
        const context = Aes128.initEnc(key);
        cbc(@TypeOf(context), context, dst, src, iv);
    }

    pub fn decrypt(dst: []u8, src: []u8, key: [Aes128.block.block_length]u8, iv: [AesDecryptCtx.block_length]u8) void {
